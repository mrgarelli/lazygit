	...
	menuItems := []*menuItem{
		{
			...
			onPress: func() error {
				return gui.WithWaitingStatus(gui.Tr.LcRunningCommand, func() error {
					if err := gui.OS.WithSpan(gui.Tr.Spans.BulkInitialiseSubmodules).Run(bulkInitCmdObj); err != nil {
						return gui.SurfaceError(err)
					}
		...
		},
		{
			...
			onPress: func() error {
				return gui.WithWaitingStatus(gui.Tr.LcRunningCommand, func() error {
					if err := gui.OS.WithSpan(gui.Tr.Spans.BulkUpdateSubmodules).Run(bulkUpdateCmdObj); err != nil {
						return gui.SurfaceError(err)
					}
		...
		},
		{
			...
			onPress: func() error {
				return gui.WithWaitingStatus(gui.Tr.LcRunningCommand, func() error {
					if err := gui.OS.WithSpan(gui.Tr.Spans.BulkDeinitialiseSubmodules).Run(bulkDeinitCmdObj); err != nil {
						return gui.SurfaceError(err)
					}
