		...
		HandleConfirm: func(remoteName string) error {
			return gui.Prompt(PromptOpts{
				...
				HandleConfirm: func(remoteUrl string) error {
					if err := gui.Git.WithSpan(gui.Tr.Spans.AddRemote).Remotes().AddRemote(remoteName, remoteUrl); err != nil {
						return err
					}
...
func (gui *Gui) handleRemoveRemote() error {
	...
	return gui.Ask(AskOpts{
		...
		HandleConfirm: func() error {
			if err := gui.Git.WithSpan(gui.Tr.Spans.RemoveRemote).Remotes().RemoveRemote(remote.Name); err != nil {
				return gui.SurfaceError(err)
			}
	...
	return gui.Prompt(PromptOpts{
		...
		HandleConfirm: func(updatedRemoteName string) error {
			if updatedRemoteName != remote.Name {
				if err := gitCommand.Remotes().RenameRemote(remote.Name, updatedRemoteName); err != nil {
					return gui.SurfaceError(err)
				}
			}
			...
			return gui.Prompt(PromptOpts{
				...
				HandleConfirm: func(updatedRemoteUrl string) error {
					if err := gitCommand.Remotes().UpdateRemoteUrl(updatedRemoteName, updatedRemoteUrl); err != nil {
						return gui.SurfaceError(err)
					}
					return gui.RefreshSidePanels(RefreshOptions{Scope: []RefreshableView{BRANCHES, REMOTES}})
				},
