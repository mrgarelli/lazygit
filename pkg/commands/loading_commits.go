...
func (c *CommitListBuilder) getMergeBase(refName string) (string, error) {
	...
	output, _ := c.OSCommand.RunExecutableWithOutput(fmt.Sprintf("merge-base %s %s", refName, baseBranch))
...
func (c *CommitListBuilder) getFirstPushedCommit(refName string) (string, error) {
	output, err := c.OSCommand.RunExecutableWithOutput(fmt.Sprintf("merge-base %s %s@{u}", refName, refName))
	if err != nil {
		return "", err
	}
